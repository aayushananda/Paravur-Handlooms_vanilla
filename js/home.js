...
function renderHomeSlider() {
  ...
  contentContainer.innerHTML = slides.map((slide, index) => `
    <div class="hero-slide-content ${index === currentSlide ? 'active' : ''}" id="hero-content-${index}">
      ...
      <button class="hero-btn">Discover the Craft</button>
    </div>
  `).join('');
...
function goToSlide(index) {
  // Hide current
  document.getElementById(`hero-bg-${currentSlide}`)?.classList.remove('active');
  document.getElementById(`hero-content-${currentSlide}`)?.classList.remove('active');
  document.querySelector(`.hero-control-btn[data-index="${currentSlide}"]`)?.classList.remove('active');

  currentSlide = index;

  // Show new
  document.getElementById(`hero-bg-${currentSlide}`)?.classList.add('active');
  document.getElementById(`hero-content-${currentSlide}`)?.classList.add('active');
  document.querySelector(`.hero-control-btn[data-index="${currentSlide}"]`)?.classList.add('active');
}
